...
function Search() {
  ...
  const [cartCount, setCartCount] = useContext<any>(CartContext);
	...
	const fetchCourse = async () => {
		setIsLoading(true);
		try {
			await useAxios().get(`/core/course-list/`).then((res) => {
				setCourses(res.data);
				setIsLoading(false);
			});
		} catch (error) {
	...
	const handleSeach = (e: { target: { value: string; }; }) => {
		...
		if (query === "") {
			...
			const course = courses.filter((course: { title: string; }) => {
						...
						<div className='col-lg-6 col-12 d-none d-lg-block'>
							<div className='d-flex justify-content-center pt-4'>
								...
								<div className='position-relative'>
									<img
										src='https://desphixs.com/geeks/assets/images/png/cta-instructor-1.png'
										alt='image'
										className='img-fluid mt-n8'
									/>
									<div className='ms-n8 position-absolute bottom-0 start-0 mb-6'>
										<img
											src='https://desphixs.com/geeks/assets/images/svg/dollor.svg'
											alt='dollor'
										/>
									</div>
									{/* img */}
									<div className='me-n4 position-absolute top-0 end-0'>
										<img
											src='https://desphixs.com/geeks/assets/images/svg/graph.svg'
											alt='graph'
										/>
									</div>
								</div>
